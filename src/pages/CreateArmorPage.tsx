...
export default function CreateArmorPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('armors').insert({
        category: slug!,
        name: data.name,
        type: data.type,
        rarity: data.rarity,
      ...
      toast({ title: 'Armadura criada com sucesso!' });
      navigate(`/armors/${slug}`);
    } catch (e: any) {
    ...
    <div className="min-h-screen flex flex-col">
      <header className="border-b border-primary/20 bg-card/50 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="font-display text-xl font-bold text-primary gold-glow">Criar Armadura</h1>
        </div>
      </header>
      ...
      <main className="flex-1 px-4 py-6">
        <div className="max-w-lg mx-auto space-y-4">
          ...
          <EquipmentFields data={data} onChange={setData} nameLabel="Nome da armadura" showDefense />
